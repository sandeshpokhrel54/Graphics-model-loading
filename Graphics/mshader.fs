...
vec3 fogColor =  vec3(0.8314, 0.9098, 0.9333);//fog color 0.8314, 0.9098, 0.9333

void main()
{    
    //normal mapping
    // vec3 rgb_normal = texture_normal1 * 0.5 + 0.5;
    vec3 normFromMap = 0.55 *  texture(texture_normal1, TexCoords).rgb;  //the scalar in the front controls the strength of the map
    ...
    //overall direction light effect
    vec4 tex = texture(texture_diffuse1, TexCoords) * vec4(ambient+diffuse+specular,1.0) /*+ texture(texture_specular1,TexCoords) * vec4(specular,1.0f)*/;
    ...
    FragColor = tex + 0.6 * vec4(pointLightEffect, 1.0); //reducing overall effect of the light

    if(fogbool ==1)
        FragColor = mix(vec4(fogColor,1.0f),FragColor,fog);
